...
## 🧩 LLMが一語を出すまでを、3つの段階に分けて見る

![どの段階でゆらがず、どの段階でゆらぐか](images/01_determinism.png)

| 段階 | 何が起きるか | ゆらぐ？ |
|---|---|---|
| **① 質問を読む** | プロンプトは毎回同じ → 内部状態（hidden state）も**毎回まったく同じ**（差ゼロ・決定的な計算） | **ゆらがない** |
| **② 次の語の確率を出す** | その状態から「次の語のオッズ」が決まる（例: 「DNA」50%／「Yes」40%／他10%）。これも**毎回同じ** | **ゆらがない** |
| **③ 確率に従って1語を引く** | その同じオッズから、**サイコロを振って実際に1語を選ぶ**。オッズは同じでも**出目は毎回違う** | **← ここでゆらぐ** |

## 💡 一見の矛盾を解く

「内部状態はゆらがないのに、語を選ぶとゆらぐ」は矛盾ではありません。

- 内部状態は「**同じ入力なら必ず同じ**」という決定的な計算の結果。状態が勝手にゆらぐのではない
- ゆらぎは、**②の同じ確率の中から③でどの語を引くか**という「**出目**」から生まれる
- そして③で違う語を引くと、次のステップの**入力が変わる**ので、それ以降の内部状態も変わっていく → こうして**文全体がゆらぐ**

> だから「ゆらぎを測る」には、〈②の確定した状態が何を表しているか〉を理解し、次に〈③以降で実際にどれだけ違う道を辿るか〉を見ればよい ― これが以降の道筋です。
