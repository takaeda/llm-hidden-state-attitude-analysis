...
売上課題では、木は深さ20で**48本の枝**に分かれました。しかし14回のフル回答を読むと、**13回までが同じ時系列系の提案**（うち8回は「時系列の分析を」、5回はより具体的に「時系列の分解を」という提案の粒度違い）で、本当に別の提案（顧客セグメント分析）は1回だけ。「48本の枝のうちどれとどれが同じ意味か」を木は教えてくれません。その**意味の束ね**を担うのが hidden state 空間です——右図（PCAによる2次元化）のとおり、全文の hidden state を見れば、読まずに「ほぼ全員同じ系統のことを言っている（小塊は提案の粒度差）＋例外1」が分かります。
