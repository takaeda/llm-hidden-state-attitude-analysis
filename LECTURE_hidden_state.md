...
- DNAの質問：点が**グループに割れる**——頭の中が、回によって違う場所にいる
  - 細かく見ると赤も2つの島に分かれていますが、これは**中身は同じ「二重らせん」で、出だしの言い回しが違うだけ**（"DNA typically…" と始めた回と、"DNA (deoxyribonucleic acid)…" と正式名称から始めた回）。**中身まで違う**のはオレンジの三角（「三重らせんもあります」派）です

> **答えの文章を読まなくても、点の散らばりを見れば「迷っているかどうか」が分かる。**

（この図は単純な2次元化〔主成分分析〕で描いたもの。本講義の道具 **toorPIA** は次のスライドで登場します）
...
質問を12問に増やし、各20回ずつ答えさせて、**全部の「頭の中」を一枚の地図**にしました。使う道具は、本講義でおなじみの**toorPIA（高次元データの可視化）**です。
...
# スライド 11: 〔演習〕自分のAIの「ゆらぎ」を観察してみよう

## 📝 やること（GPU不要・スマホでも可）

1. お好きなチャットAIに、**同じ質問を5回**してみる（毎回新しいチャットで）
2. 質問は2種類試す：
   - 事実を聞く質問（例：「光は1秒で何km進む？」）
   - 意見が分かれそうな質問（例：「リモートワークと出社、どちらが生産的？」）
3. 観察したことをメモする：答えは何通りに割れた？ 「言い回しだけ違う」のと「中身が違う」のは区別できた？

## 🤔 考える問い（任意）

- あなたの業務で「**いつも同じ答えなら、多少間違っていても使える**」場面はどこですか？
