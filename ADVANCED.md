...
- 「DNAは三重らせん？」：「二重らせんだ」×10回と「Yes、三重らせんもある」×2回に**実際に立場が割れ**、点群も**立場ごとの塊**に分かれる（二重らせん派の中は、さらに言い回しで3つの小島）→ 散らばり 0.267
...
hidden state は数千次元の点なので、そのままでは見えません。本講義で学んできた**高次元データの可視化（toorPIA）**に投入すると、全質問の点群を一枚で見渡せます。代表12問 × 各12回 ＝ 144点（スライド7と同一データ）：

![toorPIAマップ](images/03_toorpia_map.png)

## ✅ マップの読み方

- **一点に固まっている質問**：12回ぶんの状態がぴったり重なっている ＝ **答えが一貫**。ほとんどの質問がこれです
...
同じ点群を PCA と toorPIA で2次元化して並べると：

![PCAとtoorPIAの比較](images/09_pca_vs_toorpia.png)

- **グループ構造（どの点が群れるか）は両者で一致**——可視化手法を変えても結論は変わらない、という頑健性の確認
- **島どうしの距離の見た目は手法に依存**します。例えば sf07 では、PCAは点数の多い「言い回し2系統」の分裂を分散最大方向として強調しがちですが、toorPIAでは言い回し違いの2島が近くに置かれ、立場の違う島が遠い——**立場の序列（言い回し差 ＜ 立場差）に近い配置**になりました
- だからこそ本資料の**定量指標（散らばり・分離度・D_ref）は2次元図からではなく、高次元空間のコサイン距離で直接計算**しています。図は構造を掴むためのもの、数値は高次元で測るもの、という役割分担です
...
ゆらぎが大きいのは「入力が曖昧」か「モデルがその領域で際どい」かのどちらかですが、**どちらであっても自動処理を止めるのが正解**です。そこで、ゆらぎの大きい入力を検知したら「より明確な入力をお願いします」と入力者に返す・人間に回す、という**品質ゲート**の運用が考えられます（機械学習で確立された「選択的予測」のLLM版。スライド15の合議モジュールにもつながります）。
...
> つまり出だしの hidden state は、**文の途中から先の言い回しの違いには惑わされず**、立場の一貫性を映す。
> ※ただし**出だしの言い回しの違いには敏感**です。同じ立場でも書き出しが違えば（例: "DNA typically…" と "DNA (deoxyribonucleic acid)…"）散らばりに数えられます。実測では立場間の距離は出だしの言い回し差の約2倍あり序列は保たれますが、両者の完全な切り分けは未解決（→どの位置・層の状態を使うべきかは「開かれた問い」参照）。

【結論】**出力文を読まずに、非一貫を有意に検出できる**ことが確認できた（散らばり大→意味も割れている、の方向に反例なし）。出だし限定ゆえ相関は中程度で、散らばりの中身には立場の分裂と出だしの言い回し差が混在する。

---

# スライド 14: 〔よくある質問〕そもそも試行せず、確率分布を直接計算すればよいのでは？

## 🙋 もっともな疑問（その2）

Local LLM なら、次の語の確率分布そのものが forward 1回で取れます（スライド4の②）。それを枝のように展開していけば、**出だし数語の分布はサンプリングせずに厳密に計算できる**はず。実際にやってみると——

> **その通りでした。** Yes/No型の質問では、出だし3語の各分岐の確率が forward 3〜6回で厳密に求まり、サンプリング20〜40回の頻度と誤差の範囲で一致。浅く単純な領域では、**木展開の方が試行より速くて正確**です。

では、試行も hidden state も要らないのでは？ ― 複雑な課題で測ると、**2つの壁**が現れます。

![木展開の限界](images/07_tree_limit.png)

## 🧱 壁1：分岐は「深部」で始まり、以後コストが指数的に増える（左図）

オープンエンドな課題（例：売上分析の提案）では、回答の冒頭は**質問の復唱**でほぼ決定的（枝1本のまま）。本当の分岐は**7〜13語目以降**で始まり、以後は枝数が毎語 ×1.2〜1.3 で増殖します。回答全文（90語超）の深さまで掘ると枝数は桁外れになり、**木は計算不能**。一方サンプリングのコストは**回数×長さの線形**のまま——深い領域では試行が唯一の現実的な推定器です。

## 🧶 壁2：木が返すのは「文字列」の確率。意味は束ねてくれない（右図）

売上課題では、木は深さ20で**48本の枝**に分かれました。しかし14回のフル回答を読むと、**13回までが同じ時系列系の提案**（うち8回は「時系列の分析を」、5回はより具体的に「時系列の分解を」という提案の粒度違い）で、本当に別の提案（顧客セグメント分析）は1回だけ。「48本の枝のうちどれとどれが同じ意味か」を木は教えてくれません。その**意味の束ね**を担うのが hidden state 空間です——右図（toorPIAによる2次元化）のとおり、全文の hidden state を見れば、読まずに「ほぼ全員同じ系統のことを言っている（小塊は提案の粒度差）＋例外1」が分かります。

## 🧭 整理：役割分担（これが答え）

| 領域 | 最適な道具 |
|---|---|
| 浅い・単純（Yes/No型の出だし） | **木展開**（厳密・ノイズゼロ・最速） |
| 深い・複雑（実務の生成タスク） | **試行 ＋ hidden state 空間**（唯一の現実解） |

単純な質問は、計器を検証するための**モデル系**でした。検証済みの計器を持って、木が届かない複雑な実務タスクへ——それが次のスライドの話です。

---

# スライド 15: 展望 ― 複数LLMの「合議」で出力を最適化する（AIエージェント連携）

## 🧭 ここからは「構想」です ― 線引きを先に

スライド14までは、**実際に測定して確認した話**でした。最後に、この基礎技術が目指す先——複数のAIエージェント（LLMワーカー）を連携させるシステム——の構想を紹介します。どこまでが実証済みで、どこからが未検証かを明示しながら進めます。
...
# スライド 16: まとめ ― hidden state とは何だったか
...
Local LLM の**評価手法の構築**を通して、小型で安価なモデルの性能向上と実用化に貢献していく――その道は、まだ誰のものでもありません。

## 🔭 すぐ隣にある、まだ誰も解いていない問い

今日の内容の「すぐ続き」に、未解決の問いが並んでいます。一つだけ予備実験をお見せします：

![層スイープ予備実験](images/08_layer_sweep.png)

「意味や立場は、**どの層**で測るのが最適か？」――測ってみると、**指標ごとに答えが違いました**。言い回しの違いを「同じ意味」として畳み込むのは中間層（L15-24、最終層より約3割低ノイズ）、意味の分離はやや深部（L27-30）、立場の分離は最終層も健闘。**単一の答えはまだ存在しません**。この一枚の図の先が、そのまま研究になります。ほかにも：

- **中間層の toorPIA 可視化で、モデルの「意味的な認知構造」そのもの（分布のエントロピーや異方性のような量）を評価できないか？** ― 一貫性測定を超えて、モデルの認知の形を測る構想
- 復唱（エコー）区間を自動検出して除外する、タスク適応的な窓の設計
- コード生成・要約など実務の生成タスクでの一貫性検証と、合議モジュール（スライド15）の実装
- 日本語の質問・業務での再検証（今日の測定は英語）

## 🤝 一緒にやりませんか

今日、すべての答えを出す必要はありません。これからやればいいのです。道具（測り方・コード・データ）はすべてこのリポジトリに置いてあります。この問いのどれかに心が動いた人は、ぜひ声をかけてください。**その担い手として、皆さんに期待しています。**

---

# スライド 18: 〔付録〕分かりやすい一つの数字を、疑う
