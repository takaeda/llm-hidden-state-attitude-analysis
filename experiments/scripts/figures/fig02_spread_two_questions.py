...
出だし3トークンhidden state（平均プール）を、共通のPCA平面で点群表示。
...
ax1.scatter(PA[:, 0] + rng.normal(0, 0.006, len(PA)),
            PA[:, 1] + rng.normal(0, 0.006, len(PA)),
...
ax2.scatter(PB[oth, 0] + rng.normal(0, 0.006, len(oth)),
            PB[oth, 1] + rng.normal(0, 0.006, len(oth)),
            s=130, color="#d62728", alpha=0.6, edgecolors="white",
            label=f'「二重らせんだ」と答えた回 ×{len(oth)}')
ax2.scatter(PB[yes, 0] + rng.normal(0, 0.006, len(yes)),
            PB[yes, 1] + rng.normal(0, 0.006, len(yes)),
...
for ax in (ax1, ax2):
    ax.set_xlabel("2次元化した hidden state")
    ax.grid(alpha=0.3)
