...
# スライド 3: 【事実1】hidden state は「決まった」状態 ― 乱数でも魔法でもない

## 🧪 確かめたこと

同じ質問を、まったく同じ条件で何度も読み込ませて、質問を読み終えた時点の hidden state を比べました。

## ✅ 結果

> **毎回、まったく同じ値**（差はゼロ）。

hidden state は、入力に対して**決まった計算**で出てくる結果です。気まぐれでも乱数でもありません。

![hidden stateとは](images/01_what_is_hidden_state.png)

## 💡 ここで大事なこと

- 「出力がばらつく」のは、最後に**確率的に一語を選ぶ（サンプリング）段階**でだけ起きる
- 質問を読み終えた瞬間の内部状態は、毎回同じ。**まず確定した状態があり、そこから言葉を引く**、という順序

---

# スライド 4: 【事実2】hidden state は「文脈の中の意味」を表す
...
# スライド 5: 【事実3】hidden state は「次に何を言うか」をすでに持っている
...
# スライド 6: 【核心】何度も答えさせて、状態の「散らばり」を見る

## 💡 発想の転換

一回の答えではなく、**同じ質問を何度も答えさせて、その時の hidden state を集めて点群にする**。
...
# スライド 8: なぜこれが実務で重要なのか ― 一貫性 ≠ 正しさ

## 🏭 実務でLocal LLMを使うとき、本当に欲しいもの

定型業務にLLMを載せるとき、最優先は「正しさ」より「**予測可能性（一貫性）**」です。

> **毎回違う答えを返すLLMより、いつも同じ答えを返すLLMの方が、たとえ間違っていても定型業務に載せやすい。**

「いつも同じ間違いをする」モデルは、間違いを見越して運用設計できます。「毎回違う」モデルは設計できません。
...
# スライド 10: 結果 ― 出力一貫性によるLocal LLM品質評価
...
# スライド 11: 〔検証〕この「読まない一貫性」は、本当に正しいのか
...
> _（検証(a)の相関値をここに挿入：Spearman ρ = ◯◯。両端の実例も提示）_

【この一致が高ければ】出力を読まずに、意味的な一貫性を代理できる ＝ 基礎技術として成立。
【低ければ】出だし数語では後半の意味分岐を捉えきれない、という限界が分かる（窓を広げる等で調整）。

※ このページは生成中の検証結果が出次第、数値と実例を差し込みます。

---

# スライド 12: 応用 ― 複数LLMの「合議」で出力を最適化する
...
# スライド 13: まとめ ― hidden state とは何だったか
