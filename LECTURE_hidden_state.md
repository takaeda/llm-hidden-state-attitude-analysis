...
同じ質問を、まったく同じ条件で何度も読み込ませて、質問を読み終えた時点の hidden state を比べました。「ゆらぎ」は内部のどこで生まれているのかを探ります。

## ✅ 結果：内部状態は「ゆらがない」

> 質問を読み終えた時点の hidden state は、**毎回まったく同じ値**（差はゼロ）。

![ゆらぎはサンプリングの一点だけで生まれる](images/01_determinism.png)

図のとおり、12回ぶんの内部状態は一点にぴったり重なります。出力がばらつくのは、その後の**サンプリング（確率的に一語を選ぶ）**の段階だけ。hidden state は入力に対して**決まった計算**で出てくる結果で、気まぐれでも乱数でもありません。

## 💡 つまり「ゆらぎ」の出所は一箇所だけ

- 内部状態は決定的。**ゆらぎは、最後に確率的に一語を選ぶ（サンプリング）段階でだけ**生まれる
- **まず確定した状態があり、そこから言葉を確率的に引く**、という順序
- だから「ゆらぎを測る」には、まず〈この確定した状態が何を表しているか〉を理解し、次に〈言葉を引くたびに何が起きるか〉を見ればよい ― これが以降の道筋です
