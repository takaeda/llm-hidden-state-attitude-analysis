...
（補足：回答の最後まで含めると「言い回しの多様性」が混ざって膨らむので、立場の一貫性を見るには**出だし数語**で測るのがコツ。）
...
- **GPUがある場合**：`python scripts/08_consistency_metric.py --model Qwen/Qwen3-4B --load-4bit` で、一貫性スコア（D_ref・d′・規格化inconsistency）を自分のモデルで算出してみる
- **GPUが無い場合**：`experiments/results/<model>/*.json` の測定済み数値を読み、カテゴリ別の一貫性を比べる。`results/Qwen3-4B/dispersion_vectors.csv` を toorPIA に投入して散らばりを可視化する（スライド8の図の再現）
