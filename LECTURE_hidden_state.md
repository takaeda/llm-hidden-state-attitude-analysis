...
4つのLocal LLM（Qwen3-4B / Mistral-7B / Phi-4-mini / DeepSeek-R1-1.5B）に、スライド7の36問とスライド10の物差しを適用した結果です。
...
ポイントは Evaluator の中身です。ここまでで作った「読まない一貫性測定」がそのまま使えます：

- 出力文を**読まずに**選別できるので、高速・低コスト・自動化できる
- 「確立した一貫出力の分布」を**ベースマップ**として持っておき、新しい出力の状態がそこから**どれだけズレているか（異常スコア）**で非一貫な出力を弾く
- つまり、toorPIA の「ベースマップ＋追加プロット＋異常検知」という、本講義でおなじみの構成そのもので実装できる
